...
export default function Navbar() {
	...
	return (
		<header className={`fixed inset-x-0 top-0 z-20 flex items-center justify-between px-[4.5vw] py-7 text-[#f5eee3] transition-colors duration-300 max-[700px]:py-4 ${hasScrolled ? "bg-[#e08f62]" : "bg-transparent"}`}>
			<Link className="ml-[4.2vw] text-[30px] font-bold tracking-[-0.08em] max-[700px]:text-[27px]" href="/#top" aria-label="DraftBIM Services home">
				DraftBIM<span className="text-[#e08f62]">.</span>
			</Link>
			<button
				className="hidden cursor-pointer border-0 bg-transparent p-2 max-[700px]:ml-auto max-[700px]:block"
				type="button"
			...
			>
				<span className={`block h-px w-6 bg-[#f5eee3] transition-transform duration-300 ${isMenuOpen ? "translate-y-2 rotate-45" : ""}`} />
				<span className={`mt-1.5 block h-px w-6 bg-[#f5eee3] transition-opacity duration-200 ${isMenuOpen ? "opacity-0" : "opacity-100"}`} />
				<span className={`mt-1.5 block h-px w-6 bg-[#f5eee3] transition-transform duration-300 ${isMenuOpen ? "-translate-y-1.5 -rotate-45" : ""}`} />
			</button>
			<nav id="primary-navigation" className={`${isMenuOpen ? "flex" : "hidden"} absolute left-1/2 -translate-x-1/2 items-center gap-4 text-[13px] uppercase tracking-[0.14em] max-[700px]:left-auto max-[700px]:right-[4.5vw] max-[700px]:top-full max-[700px]:w-max max-[700px]:translate-x-0 max-[700px]:flex-col max-[700px]:items-stretch max-[700px]:gap-1 max-[700px]:border max-[700px]:border-[#40545b] max-[700px]:bg-[#17232b] max-[700px]:p-2 max-[700px]:text-[10px] min-[701px]:flex`}>
				{navItems.map((item) => {
					...
					return (
						<Link
							key={item.key}
							className={`group relative px-5 py-3 text-[#f5eee3] transition-colors max-[700px]:px-3 max-[700px]:py-2 ${hasScrolled ? "hover:text-[#173d46]" : "hover:text-[#e08f62]"}`}
							href={item.href}
						...
						>
							...
							{isActive && (
								<motion.span
									...
									className={`absolute bottom-1 left-5 right-5 h-0.5 max-[700px]:left-3 max-[700px]:right-3 ${hasScrolled ? "bg-[#173d46]" : "bg-[#e08f62]"}`}
			...
			<p className={`ml-auto hidden text-[10px] uppercase tracking-[0.18em] md:block ${hasScrolled ? "text-[#f5eee3]" : "text-[#9aabb0]"}`}>
