...
function BuildingModel() {
  ...
  const slabColors = ["#d8e0dc", "#eef2e8", "#9da9a0", "#d8e0dc", "#eef2e8"];
    ...
    <group ref={model} position={[0, -1.15, 0]} rotation={[0, -0.18, 0]} scale={1.28}>
      <Float speed={1.8} rotationIntensity={0.07} floatIntensity={0.28}>
        <mesh position={[0, -0.1, 0]} rotation={[0.05, 0, -0.08]} receiveShadow>
          ...
          <meshStandardMaterial color="#173d46" roughness={0.72} metalness={0.35} />
        </mesh>
        ...
        <mesh position={[-1.23, 0.8, 0.08]} castShadow>
          ...
          <meshPhysicalMaterial color="#9da9a0" roughness={0.17} metalness={0.64} clearcoat={0.8} />
        </mesh>
        <mesh ref={core} position={[0, 1.65, 0.1]} castShadow>
          <boxGeometry args={[1.05, 3.35, 0.95]} />
          <meshPhysicalMaterial color="#eef2e8" roughness={0.14} metalness={0.52} clearcoat={1} />
        </mesh>
        <mesh position={[1.2, 1.04, 0.08]} castShadow>
          <boxGeometry args={[0.76, 2.25, 1.42]} />
          <meshPhysicalMaterial color="#8ba9b2" roughness={0.2} metalness={0.7} clearcoat={0.7} />
        </mesh>

        {windows.map((window) => (
          <mesh key={`window-${window}`} position={[-0.52 + (window % 4) * 0.36, 0.2 + Math.floor(window / 4) * 0.48, 1.09]}>
            <boxGeometry args={[0.2, 0.3, 0.025]} />
            <meshStandardMaterial color={window % 2 ? "#79c6d0" : "#d8f3f0"} emissive="#173d46" emissiveIntensity={0.12} metalness={0.2} roughness={0.08} />
          </mesh>
        ))}

        <mesh position={[0.9, 0.95, 0.82]} rotation={[0, Math.PI / 2, 0]}>
          <planeGeometry args={[1.65, 1.9]} />
          <meshPhysicalMaterial color="#9ee0e1" transparent opacity={0.28} roughness={0.05} metalness={0.2} transmission={0.35} />
        </mesh>
      </Float>
    </group>
  );
...
function BuildingScene() {
  return (
    <Canvas camera={{ position: [5.1, 3.1, 6.3], fov: 34 }} dpr={[1, 2]} shadows gl={{ antialias: true, alpha: true }}>
      ...
      <directionalLight castShadow position={[3, 4, 4]} intensity={3.5} color="#f5eee3" />
      ...
      <Sparkles count={90} scale={7} size={1.5} speed={0.18} color="#e08f62" opacity={0.42} />
      <BuildingModel />
  ...
  return (
    <section className="building-section" id="building-study">
      ...
      <div className="building-heading">
        <p className="building-kicker"><span>02</span> Planning approvals</p>
        <h2>Town planning<br /><em>approvals.</em></h2>
        <p className="building-description">
          At DraftBIM, we make obtaining building plan approvals easy. Our team guides you
          through every step, ensuring a smooth process for your architectural projects in Odisha.
          From submission-ready drawings to compliance coordination, we help keep your project moving.
        </p>
