...
function SceneObject({ isExploring }: { isExploring: boolean }) {
  ...
  return <group ref={group} position={[0, -1.2, 0]}><Float speed={1.1} rotationIntensity={0.08} floatIntensity={0.28}>
    {floors.map((floor) => <mesh key={`floor-${floor}`} position={[0, floor * 0.55, 0]} castShadow>
      ...
      <meshPhysicalMaterial color="#DDEAF2" roughness={0.22} metalness={0.58} clearcoat={0.7} />
    </mesh>)}
    {floors.slice(0, 4).flatMap((floor) => columns.map((x) => <mesh key={`column-${floor}-${x}`} position={[x, floor * 0.55 + 0.26, -0.75]}>
      ...
      <meshStandardMaterial color="#B8CFDF" metalness={0.45} roughness={0.3} />
    </mesh>))}
    <mesh ref={core} position={[0, 1.35, 0.18]} castShadow>
      <boxGeometry args={[0.72, 2.5, 0.62]} />
      <meshPhysicalMaterial color="#8CA7BD" roughness={0.18} metalness={0.76} clearcoat={1} />
    </mesh>
    <mesh position={[0, 2.82, 0]} rotation={[0, 0, Math.PI / 4]}>
      <boxGeometry args={[0.2, 0.2, 0.2]} />
      <meshBasicMaterial color="#204C72" wireframe />
    </mesh>
    <mesh position={[0, 1.1, 1.08]}>
      <boxGeometry args={[2.35, 1.7, 0.025]} />
      <meshStandardMaterial color="#8AA5BC" transparent opacity={0.32} metalness={0.2} roughness={0.08} />
    </mesh>
    <mesh position={[0, 0.7, 0.28]} rotation={[Math.PI / 2, 0, 0]}>
      <torusGeometry args={[0.72, 0.025, 12, 64]} />
      <meshBasicMaterial color="#204C72" />
    </mesh>
    ...
    <mesh position={[-0.86, 1.05, 0.96]} rotation={[0, Math.PI / 2, 0]}>
      <cylinderGeometry args={[0.035, 0.035, 1.65, 12]} />
      <meshBasicMaterial color="#B8CFDF" />
    </mesh>
  </Float></group>;
}
...
function Scene({ isExploring }: { isExploring: boolean }) {
  return <Canvas camera={{ position: [0, 0.15, 5.6], fov: 38 }} dpr={[1, 2]} gl={{ antialias: true, alpha: true }}>
    <Suspense fallback={null}>
      <ambientLight intensity={0.35} /><directionalLight position={[3, 4, 4]} intensity={3.5} color="#F7F9FA" />
      <pointLight position={[-3, -1, 2]} intensity={18} distance={8} color="#204C72" /><pointLight position={[3, 1, -1]} intensity={12} distance={7} color="#9DBACD" />
      <SceneObject isExploring={isExploring} /><Sparkles count={90} scale={7} size={1.5} speed={0.18} color="#204C72" opacity={0.42} />
...
export default function Home() {
  ...
  return <main className="hero-shell">
    ...
    <section className="hero-content" id="top"><div className="hero-copy"><p className="eyebrow"><span>01</span> Design your dream home</p><h1>Welcome to<br /><em>Draft BIM</em></h1><p className="hero-description">Architectural Engineering Consultants</p><div className="service-list" aria-label="DraftBIM services"><span>3D BIM Modelling</span><span>Architectural + Structural</span></div><button className="explore-button" type="button" onClick={() => setIsExploring((value) => !value)}><span>{isExploring ? "Close model" : "Explore model"}</span></button></div>
      <div className="scene-wrap" aria-label="Interactive 3D building information model" role="img">
        {webgl === true && <Scene isExploring={isExploring} />}
        {webgl === false && (
